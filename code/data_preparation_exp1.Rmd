...
# Define how to load the data: from the "data" folder in GitHub or from an external folder (eg., local folder) using read_bids.R
data_option <- "github" # or "read_bids"

# Parameters for the read_bids option
if (data_option == "read_bids") {
  projdir <- "/Users/au342995/Documents/MINDLAB2022_CalibrationTGI"
  #projdir <- "/mnt/cortex/bpp/BPP_BIDS/MINDLAB2022_CalibrationTGI"
...
```
# How many participants did complete TGI thresholding task?
```{r}

ses1_df <- df_avail %>% filter(ses == 1 & fastEst > 0)
ses2_df <- df_avail %>% filter(ses == 2 & fastEst > 0)
participants_both_ses <- intersect(ses1_df$sub, ses2_df$sub)
participants_both_ses <- sort(as.numeric(participants_both_ses))

# Print the participants with both ses1 and ses2
cat("N = ", length(participants_both_ses), "completed TGI thresholding task", "\n")
