...
```{r warning=FALSE}
source(here::here('code','plot_thresholds_v1.R'))
source(here::here('code','plot_ratings_subject_v1.R'))
...
walk(sublist, function(sub_n) {
  ...
  for (seslist in 1:2) {
    ...
    # Plot the session
    thr_plots[[seslist]] <- plot_thresholds_v1(vas_sub, fe_sub, psi_sub, title)
    vas_plots[[seslist]] <- plot_ratings_subject_v1(df_vas)
  }
  ...
  #ggsave(g, file = filepath, width = 20, height = 10, dpi = 100, units = "cm")
})
```
