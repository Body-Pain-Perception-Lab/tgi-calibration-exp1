...
data_option <- "read_bids" #"github" # or "cortex"
...
if (data_option == "read_bids") {
  projdir <- "/Users/au706606/Downloads/MINDLAB2022_CalibrationTGI"
  sub <- 1:28     # TO UPDATE BASED ON DATA COLLECTION
  ses <- 1:2
  dat <- "beh"
  save <- TRUE
...
vas_temps <- vas %>%
  ...
  summarize(temp_cold = mean(target_cold), temp_warm = mean(target_warm))
