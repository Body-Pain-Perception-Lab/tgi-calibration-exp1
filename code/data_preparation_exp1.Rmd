...
df_fe <- crossing(sublist, seslist) %>%
  pmap_dfr(function(sublist, seslist) {
    psi_sub <- psi %>% filter(sub == sublist & ses == seslist) # define CPT and HPT for one participant
...
# Select the last row with non-NA data
df_psi <- psi %>%
...
walk(sublist, function(sub) {
  ...
  for (seslist in 1:2) {
    ...
    vas_sub <- df_vas %>% filter(sub == sublist & ses == seslist)
    fe_sub <- df_fe %>% filter(sub == sublist & ses == seslist)
    psi_sub <- df_psi %>% filter(sub == sublist & ses == seslist)
    
    # Set title for the session
    title <- paste('fast TGI', paste('Subject', sublist), paste('Session', seslist), sep = " ")
  ...
  g1 <- ggarrange(plots[[1]], plots[[2]], labels = c("A", "B"), ncol = 2, nrow = 1)

  # Generate figure combining p1 and p2
  filename <- file.path(projdir, 'figs', paste0('sub-', str_pad(sub, 4, pad = '0'), '.pdf'))
  #ggsave(g1, file = filename, width = 20, height = 10, dpi = 100, units = "cm")
})
```
