...
vars.task.Twarmmin           = 35;   % Highest limit of cold temperatures
vars.control.whichMethodCW   = 1; %Use Method of Limits 2. Use Psi 1
[vars.task.TcoldTGI,~,...
    vars.task.Tmin,...
    vars.task.Tmax]          = computeTGItemps(vars,0.5,0.5);   % Fixed TGI cold temperature. - Extracted at 50% of the valid temperature interval in the alpha curve

%Trials
vars.task.NTrialsTotal       = 160; % Total number of trials (It DOES include the habituation trials), so the total number of trials in Psi is this value subtracted by vars.task.NTrialsHab 
...
vars.task.NTrialsHab         = 10; %Number of habituation trials - Put 0 if you don't want this option

% Sequence
vars.task.TrialStimArray     = [2];% 0=cold, 1=warm, 2=TGI
...
vars.psi.priorBetaRange =  linspace(log10(0.001),log10(1),vars.psi.PriorGrain); %Use log10 transformed values of beta (slope) parameter in PF
...
%Define non-uniform Priors, centered around Multidimensional values (alpha and slope)
vars.psi.prior={ones(length(vars.psi.priorAlphaRange{1}),length(vars.psi.priorBetaRange)),...
    ones(length(vars.psi.priorAlphaRange{2}),length(vars.psi.priorBetaRange)),...
    PAL_pdfNormal(vars.psi.priorAlphaRange{3},fastCalcYs(fast, vars.task.TcoldTGI, 0.5,'margMean'),3).*PAL_pdfNormal(vars.psi.priorBetaRange,-1/out1.Results.S_m(end),0.5)}; %Put new priors only for TGI
%     ones(length(vars.psi.priorAlphaRange{3}),1).*PAL_pdfNormal(vars.psi.priorBetaRange,-1/out1.Results.S_m(end),1)}; %Put new priors only for TGI
vars.psi.prior=cellfun (@(x) x./sum(sum(x)),vars.psi.prior,'UniformOutput',false);

%Plot prior distribution
% figure
% subplot 211
% plot(vars.psi.priorAlphaRange{3},PAL_pdfNormal(vars.psi.priorAlphaRange{3},fastCalcYs(fast, vars.task.TcoldTGI, 0.5,'margMean'),3))
% title('Threshold')
% subplot 212
% plot(vars.psi.priorBetaRange,PAL_pdfNormal(vars.psi.priorBetaRange,-1/out1.Results.S_m(end),0.5))
% title('Slope')
clear out1 fast

vars.psi.PM = cellfun(@(x,y,z) PAL_AMPM_setupPM('priorAlphaRange',x,...
    ...
    vars.psi.StimArray,...
    vars.psi.prior,...
    'UniformOutput',false);%'marginalize','lapse');
...
switch vars.control.language
    case 1 %English
        ...
        vars.instructions.whichQuestion = [1 1]; %Enable or disable question
